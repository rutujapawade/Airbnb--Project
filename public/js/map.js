...
// Database ke coordinates nikalne ke liye (Agar coordinates nahi hain toh Maldives ka default set hoga)
let coordinates = [3.2028, 73.2207]; // Default Maldives [Lat, Lng]

if (listing && listing.geometry && listing.geometry.coordinates && listing.geometry.coordinates.length === 2) {
    // GeoJSON me coordinates [Longitude, Latitude] hote hain, par Leaflet ko [Latitude, Longitude] chahiye hota hai
...
// 1. Map container initialize karein
const map = L.map('map').setView(coordinates, 10);

// 2. OpenStreetMap Tiles load karein (Yeh poori tarah free hai)
L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
    maxZoom: 19,
    attribution: '&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
}).addTo(map);

// 3. Map par location marker pin drop karein
L.marker(coordinates).addTo(map)
    .bindPopup(`<b>${listing.title}</b><br>${listing.location}, ${listing.country}`)
    .openPopup();

// Zoom functions jo aapke buttons ke liye kaam karenge
function zoomin() {
    map.zoomIn();
}
function zoomout() {
